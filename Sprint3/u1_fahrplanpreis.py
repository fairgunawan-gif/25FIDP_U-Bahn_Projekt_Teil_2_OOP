...
class PreisLogik:
    ...
    def ticket_typ(self, anzahl: int) -> str:
        """Bestimmt den Tickettyp anhand der Stationsanzahl."""
        if anzahl <= 3:
            return "Kurzstrecke"
        elif anzahl <= 8:
    ...
    def berechne_preis(self, ticket_typ, ermaessigung="Nein", barzahlung="Nein", einzelfahrt="Nein") -> float:
        """Berechnet den endgültigen Preis basierend auf der Wahl."""
        grundpreis = (
            self.PREISE_EINZEL[ticket_typ]
            if einzelfahrt == "Ja"
            else self.PREISE_MEHRFAHRT[ticket_typ]
        )
        ...
        return round(grundpreis * (1 + prozent_summe / 100), 2)
...
ja_nein_completer = WordCompleter(["Ja", "Nein"], ignore_case=True)

def station_prompt(text):
    return prompt(text, completer=station_completer)

def ja_nein_prompt(text):
    return prompt(text, completer=ja_nein_completer).strip().capitalize()
...
def dialog(u1):
    ...
    # --- Ticketoptionen ---
    print("\n--- Ticketoptionen ---")
    einzelfahrt  = ja_nein_prompt("Einzelfahrt?       (Ja/Nein): ")
    ermaessigung = ja_nein_prompt("Ermäßigung?        (Ja/Nein): ")
    barzahlung   = ja_nein_prompt("Barzahlung?        (Ja/Nein): ")
    ...
    print(f"Stationen:     {anzahl_station}")
    print(f"\n--- Ticket ---")
